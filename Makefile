...
bin/terraform: bin ## install terraform
	LATEST=$$(curl --silent "https://api.github.com/repos/hashicorp/terraform/releases/latest" | jq ".. .tag_name? // empty" | tr -d '"v'); curl -L https://releases.hashicorp.com/terraform/$${LATEST}/terraform_$${LATEST}_${OS}_${ARCH}.zip --output $@.zip
	unzip -o $@.zip -d ${@D}
	rm -f $@.zip
...
pre-commit: deps bin/terraform ## setup pre-commit
